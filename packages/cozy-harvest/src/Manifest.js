...
export const Manifest = {
  ...
  sanitizeIdentifier: manifest => {
    for (let inp in manifest.fields)
      if (manifest.fields[inp].role === 'identifier') return manifest
  ...
  sanitizeIsRequired: manifest => {
    for (let inp in manifest.fields)
      if (roleShouldBeRequired.indexOf(manifest.fields[inp].role) > -1)
        manifest.fields[inp].isRequired = true
  ...
  sanitizeEncrypted: manifest => {
    for (let inp in manifest.fields)
      if (legacyEncryptedFields.indexOf(inp) > -1)
        manifest.fields[inp].encrypted = true
